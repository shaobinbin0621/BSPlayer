...
public class BSPlayerControlBar: UIView {
	...
	override init(frame: CGRect) {
		...
		slider.setThumbImage(UIImage.init(named: "dot")?.scalingToSize(size: CGSize.init(width: 15, height: 15)), for: .normal)
...
//		speedBtn.layer.borderColor = UIColor.white.cgColor
	...
	private func setPortrait() {
		...
		x = playBtn.right + 10
		w = (totalTimeLabel.text == nil ? "00:00" : totalTimeLabel.text)!.getTextWidth(fontSize: totalTimeLabel.font.pointSize, h: 20) + 3
		...
		w = currentTimeLabel.width
		x = fullScreenBtn.left - 10 - w
		totalTimeLabel.frame = CGRect.init(x: x, y: y, width: w, height: h)
		
		x = currentTimeLabel.right + 10
		y = height/2 - 1
		h = 2
		w = totalTimeLabel.left - currentTimeLabel.right - 10 * 2
		progressView.frame = CGRect.init(x: x, y: y, width: w, height: h)
		
		x = progressView.left - 3
		w = w + 6
		y = y + (progressView.height == 2 ? 0 : 1)
//		y = y + 1
	...
	private func setLandscape(){
		var x: CGFloat = 10
		var y: CGFloat = 10
		var w: CGFloat = (totalTimeLabel.text == nil ? "00:00" : totalTimeLabel.text)!.getTextWidth(fontSize: totalTimeLabel.font.pointSize, h: 20) + 3
		var h: CGFloat = 15
		currentTimeLabel.frame = CGRect.init(x: x, y: y, width: w, height: h)
		...
		x = 0
		y = currentTimeLabel.top + currentTimeLabel.height + 10
		w = width
		h = 2
		progressView.frame = CGRect.init(x: x, y: y, width: w, height: h)
		...
		x = currentTimeLabel.left
		y = currentTimeLabel.top + currentTimeLabel.height + 10 + 10 + 7.5
		w = 30
		h = 30
		playBtn.frame = CGRect.init(x: x, y: y, width: w, height: h)
